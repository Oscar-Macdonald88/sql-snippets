...
INSERT INTO #Jobs (jobid) SELECT jobid = convert(varchar(50),job_id) FROM msdb_restore.dbo.SysJobs WITH (NOLOCK)
...
    SELECT @JobName = name FROM msdb_restore.dbo.sysjobs_view WHERE Job_ID = @JobID
    SELECT @JobCategory =   sc.name, @JobCategoryClass = category_class FROM msdb_restore.dbo.sysjobs sj
							INNER JOIN msdb_restore.dbo.syscategories sc
...
    FROM msdb_restore.dbo.sysjobs sj 
    INNER JOIN msdb_restore.dbo.syscategories sc
        ON sc.category_id = sj.category_id
    LEFT OUTER JOIN msdb_restore.dbo.sysoperators so
...
    SELECT @MaxSteps = MAX(step_id) FROM msdb_restore.dbo.sysjobsteps WHERE Job_ID = @JobID
...
                    @flags = flags
            FROM msdb_restore.dbo.sysjobsteps WHERE Job_ID = @JobID        
...
    FROM msdb_restore.dbo.sysjobschedules sjs WITH (NOLOCK)
...
                        FROM msdb_restore.dbo.sysjobschedules sjs WITH (NOLOCK)
                        INNER JOIN msdb_restore.dbo.sysschedules ss WITH (NOLOCK) ON sjs.schedule_id = ss.schedule_id
...
    FROM msdb_restore.dbo.sysjobservers WHERE Job_ID = @JobID
