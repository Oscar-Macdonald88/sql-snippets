-- https://www.sqlservercentral.com/articles/how-to-restore-an-agent-job-that-was-dropped
-- Note: If a specific job doesn't have notifications (emails) set up, you will need to remove the sc and so collumns and joins from the script, otherwise the job creation script output will be missing the sp_add_job stored proc
...
+ ', ' + '@notify_email_operator_name='+ CAST(so.name AS NVARCHAR(MAX))
...
FROM dbo.sysjobs AS sj 
JOIN dbo.syscategories AS sc ON sj.category_id = sc.category_id
JOIN dbo.sysoperators AS so ON sj.notify_email_operator_id = so.id
WHERE sj.name = @JobName;
-- 3. Add job steps
INSERT INTO #JOBCREATION
